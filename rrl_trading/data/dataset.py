...
def get_buy_and_hold_df(data: DataFrame) -> DataFrame: 
    ...
    df = data.loc[:, data.columns.str.contains("Close")].pct_change()
    df.loc[:, "returns"] = df.sum(axis=1) / df.shape[1]
    df.loc[:, "cumulative_returns"] = calc_cumulative_returns(df.loc [:, "returns"])

    df = df.loc[:, ["returns", "cumulative_returns"]]
    return df
